...
function preload() {
  ...
  for (let i = 1; i <= numImagenesMallas; i++) {
    let path = 'Mallas/Textura' + i + '.png';
    imagenesMalla.push(loadImage(path));
  }
...
class Puntos {
  constructor() {
    this.imagenes = imagenesPuntos; // Array de imágenes cargadas
  ...
  update(volFiltrado) {
    ...
    if (volFiltrado >= 0.40 && !this.visible) {
